...
class EmailServices {
    ...
    sendResetPasswordMail = async (email: string): Promise<TEmailResponse> => {
        // Get user by id
        const user = await User.findOne({ email });

        if (!user) {
            logger.e(
                `Error while sendResetPasswordMail. User with email=${email} not found.`
            );
            return {
                success: false,
                error: "no_user_found",
        ...
        console.log(process.env.websiteUrl + "/reset-password/" + id);
